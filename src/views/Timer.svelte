...
{:else if $recipe.ingridients.coffee}
  <div class="recipe-info">
    ...
    <div class="recipe-pad recipe-grind">
      <i>{@html grind}</i><span
        >{getGrindLevel($recipe.ingridients.grind, $translations)}</span
      >
    </div>
    <div class="recipe-pad recipe-temp">{$recipe.ingridients.temp}°</div>
    <div class="recipe-pad recipe-time">
      <i>{@html time}</i>{toMSS($recipe.ingridients.time)}
    </div>
        ...
        {:else}
          {#if $timer.step !== null && $recipe.steps[$timer.step].type === "pour"}
            <div class="timer-top" transition:scale|local>
              <span
                class="step-water"
                class:inverted={($timer.water -
                  calculateWater($recipe, $timer.step)) /
                  $recipe.steps[$timer.step].amount >
                  0.9}
              >
